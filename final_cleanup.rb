...
files_to_check = {
  ...
  'DrawingManager.swift' => 'Maps/DrawingManager.swift',
  'AddressSearchManager.swift' => 'Managers/AddressSearchManager.swift'
}

puts "Final cleanup - removing all duplicates..."
...
managers_group = main_group['Managers'] || main_group.new_group('Managers')
...
# Add Map files
['AdvancedWorkOrderMapView.swift', 'DrawingManager.swift'].each do |file|
  file_ref = maps_group.new_reference(files_to_check[file])
  project.targets.first.add_file_references([file_ref])
  puts "Added: #{file} to Maps group"
end
...
# Add Manager file
file_ref = managers_group.new_reference(files_to_check['AddressSearchManager.swift'])
project.targets.first.add_file_references([file_ref])
puts "Added: AddressSearchManager.swift to Managers group"
